...
all_predictors = c('version','review_time','time_diff','gender','experience','protocol')
...
for (this_journal in all_journals){
  for (this_predictor in all_predictors){ # loop through predictors
    ...
    if(this_journal=='Epidemiology' & this_predictor == 'protocol'){next} # no protocols at Epidemiology (all version 1)
